const UserData = require('../../../lib/UserData.js');

module.exports = async function (client, plugin, config, projectAlias, data) {
    const { discordUserId } = data || {};

    if (!discordUserId) {
        return { success: false, error: 'Nutzer-ID fehlt.' };
    }

    if (plugin.logic && typeof plugin.logic.getBirthdayForUser === 'function') {
        return await plugin.logic.getBirthdayForUser(client, plugin, discordUserId);
    }

    const userData = await UserData.get(discordUserId);
    const birthdayData = userData.getPluginData('birthday', plugin.id);

    return {
        success: true,
        birthday: birthdayData || null
    };
};

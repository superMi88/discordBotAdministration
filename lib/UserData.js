...
class UserData {
    ...
    async load() {
        ...
        if (doc) {
            ...
            this.globalName = doc.globalName || "";
            this.pluginData = doc.pluginData || {};
            
            // Populate currency Proxy without triggering modification flags
            // Prioritize currencyData as per user instructions
            const currencyData = doc.currencyData || {};
            const currency = doc.currency || {};
            const source = Object.keys(currencyData).length > 0 ? currencyData : currency;
    ...
    static async find(query, sort = null, limit = null) {
        ...
        return docs.map(doc => {
            ...
            user.globalName = doc.globalName || "";
            user.pluginData = doc.pluginData || {};
            
            // Avoid proxy tracking during mass load
            // Prioritize currencyData
            const currencyData = doc.currencyData || {};
            const currency = doc.currency || {};
            const finalSource = Object.keys(currencyData).length > 0 ? currencyData : currency;
